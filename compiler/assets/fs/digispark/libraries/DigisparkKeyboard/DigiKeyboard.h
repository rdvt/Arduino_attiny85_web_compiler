...
// El objeto V-USB precompilado usa el modo de descriptores dinámicos del core.
// DigiKeyboard tradicional solo implementaba usbFunctionSetup, así que al usar
// ese objeto quedaba un símbolo `usbFunctionDescriptor` sin resolver. Estos dos
// descriptores son los mismos del protocolo HID de teclado y permiten reutilizar
// el objeto V-USB validado sin mezclar DigiMouse y DigiKeyboard en el sketch.
const unsigned char digiKeyboardDeviceDescriptor[] PROGMEM = {
  18, 1, 0x01, 0x01, USB_CFG_DEVICE_CLASS, USB_CFG_DEVICE_SUBCLASS, 0,
  8, USB_CFG_VENDOR_ID, USB_CFG_DEVICE_ID, USB_CFG_DEVICE_VERSION,
  0, 0, 0, 1,
};
...
extern "C"{
  ...
  uchar usbFunctionDescriptor(struct usbRequest *rq) {
    ...
    if (rq->bRequest != USBRQ_GET_DESCRIPTOR) return 0;
    if (rq->wValue.bytes[1] == USBDESCR_DEVICE) {
      usbMsgPtr = (uchar *)digiKeyboardDeviceDescriptor;
      return sizeof(digiKeyboardDeviceDescriptor);
    }
    if (rq->wValue.bytes[1] == USBDESCR_HID_REPORT) {
      usbMsgPtr = (uchar *)usbHidReportDescriptor;
      return USB_CFG_HID_REPORT_DESCRIPTOR_LENGTH;
    }
    return 0;
  }
